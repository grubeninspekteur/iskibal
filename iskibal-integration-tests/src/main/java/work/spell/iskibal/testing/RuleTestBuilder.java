 ...
 * <p>
 * Orchestrates the full pipeline: Parse → Analyze → Generate → Compile → Instantiate
 *
 * <pre>
...
public class RuleTestBuilder {
   ...
   *
   * @param source the Iskara rule source code
   ...
   *
   * @param facts the fact instances
   * @return this builder
   */
   ...
   *
   * @param globals the global instances
   * @return this builder
   */
   ...
   * <p>
   * This allows tests to compile once and instantiate multiple times with different arguments,
   * improving performance for parameterized tests.
   * <p>
  ...
  public CompiledRuleTemplate compile() {
    ...
    if (!codegenResult.isSuccess()) {
      return new CompiledRuleTemplate.Failure(
          new RuleTestResult.CodegenFailure(codegenResult.getErrors()));
    }
    ...
    InMemoryCompilationResult compileResult = memCompiler.compile(fullyQualifiedName,
        generatedSource);
  ...
  public RuleTestResult build() {
    ...
    if (!parseResult.isSuccess()) {
      List<String> errors = parseResult.getDiagnostics().stream().map(Object::toString).toList();
    ...
    try {
      ...
      return new RuleTestResult.CompilationFailure(
          List.of("Failed to instantiate rules: " + e.getMessage()),
          generatedSource);
    }
  }
