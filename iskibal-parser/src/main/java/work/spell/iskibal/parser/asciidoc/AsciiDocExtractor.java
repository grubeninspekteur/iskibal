...
public class AsciiDocExtractor {
    ...
    private static final Pattern SOURCE_BLOCK_START = Pattern.compile(
            "^\\[source,iskara(?:,\\.([a-z-]+))?\\]\\s*$",
            Pattern.MULTILINE
    );
    ...
    /**
     * Extracts all Iskara content from an AsciiDoc document and combines it
     * into a single Iskara source string.
     */
    public String extractIskara(String asciidoc) {
        ...
        // Sort by type priority: imports first, then facts/globals/outputs, then data tables, then rules
        blocks.sort((a, b) -> {
    ...
    /**
     * Extracts tables ([.facts], [.outputs], [.globals], [.data-table], [.decision-table]).
     */
