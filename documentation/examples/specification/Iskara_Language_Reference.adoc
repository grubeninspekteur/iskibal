...
Nevertheless, since Iskara also shaped the Iskibal rule model design, we need to enforce some properties that a rule compiler has to maintain.
...
[source,iskara]
----
let single := 'Single quotes work and \'do what they should\''
let double := "Double quotes \"also\" work"
----

===== Template strings and string concatenation
...
The template placeholders can contain any expression, including arithmetic operations:

[source,iskara]
----
let price := 100
let tax := 19
message := $"Total: ${price + (price * tax / 100)} EUR (incl. ${tax}% tax)"
----

Non-string values inside template placeholders are automatically converted to their string representation.

If the string is not preceded by `$`, any template placeholders are treated as literals:

[source,iskara]
----
let notExpanded := "This ${variable} is not expanded"
let expanded := $"This ${variable} is expanded"
----
...
Iskara has lists `#()`, maps `#[]`, and sets `#{}`.
The `#` prefix distinguishes collection literals from block expressions.
...
Blocks are parametrized expressions similar to lambdas or closures in other languages.
They are delimited by square brackets `[]` and can have explicit parameters or use an implicit parameter.

==== Explicit parameter blocks

When a block needs to name its parameters explicitly, use the colon-prefixed parameter syntax:
...
[source,iskara]
----
pairs each: [:key :value | log: $"${key} = ${value}"]
----

==== Implicit parameter blocks (shorthand)

When a block has exactly one parameter, you can omit the parameter declaration and use `it` implicitly.
The `[| expression]` syntax makes the expression's receiver the implicit `it` parameter:

[source,iskara]
----
items where: [| active]           // equivalent to: [:it | it active]
items where: [| price > 10]       // equivalent to: [:it | it price > 10]
----

The shorthand syntax works by treating the first identifier as a unary message sent to `it`:

- `[| active]` sends the `active` message to `it`
- `[| price > 10]` sends the `price` message to `it`, then compares with 10

==== Statement blocks

For blocks without parameters that contain multiple statements, just omit the parameters:

[source,iskara]
----
condition ifTrue: [ result := "yes" ]
----

This is useful in decision table aliases where no cell value is passed.
...
You can use `[]` for "list of", `{}` for "set of", and `[<key type>:<value type>]` for "map of".
...
A rule for that could be implemented using a map:
...
when
  ...
  let maxWeight := weightByType at: Vehicle.type
...
rule VEHICLE_OVERWEIGHT "Vehicle is over weight limit"
when
  let maxWeight := WeightLimits at: Vehicle.type
  Vehicle.weight > maxWeight
then
  errors add: $"Your vehicle exceeds the maximum weight of ${maxWeight}."
end
...
when:
  People where: [:p |
    Customer.givenName = p at: "Given Name",
    Customer.familyName = p at: "Family Name",
    Customer.dateOfBirth = p at: "Date of birth"
    ] exists
then:
  Discounts add: "Birthday discount" withPercent: 15
----
...
NOTE: The shorthand block syntax `[| ...]` is for blocks with an _implicit_ parameter named `it` (used with collection operations like `where:` and `each:`).
For decision table aliases that don't need the cell value, use `[ ... ]` without the pipe.
