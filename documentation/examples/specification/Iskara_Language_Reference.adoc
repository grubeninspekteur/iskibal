...
For embedding Iskara in AsciiDoc and therefore externalizing blocks, see the related document.
...
For convenience, collections support the following messages:
...
If the block has exactly one parameter and you want to send a message to it, you can use the shorthand block syntax by omitting the parameter:
...
Only the rule compiler for a statically compiled language can now the actual return type of an expression and raise an error.
...
It is up to the rule compiler or engine how to implement a template rule, but each entry of a template rule data table _must_ lead to a separate rule firing.
...
Aliased conditionals/consequences are not concatenated with the cell, instead they receive a special `param` local variable holding the cell content as its first value.
...
then:
  param0 := "blue"
  messages add: $"Did you know you can get a birthday paint in ${param0} for your car?"
end
----
