...
IMPORTANT: When combining roles with named attributes in AsciiDoc attribute lists, the role shorthand (`.rolename`) must come _before_ any named attributes like `cols=`.
For example, use `[.facts,cols="1,1,3"]` instead of `[cols="1,1,3",.facts]`.
AsciidoctorJ silently drops the role when it appears after a named attribute.
Alternatively, use the long form `role="facts"` which is position-independent.
...
Alias bodies use standard Iskara block syntax.
Aliases that do not receive a cell value use `[ ... ]` (statement block).
Aliases that receive a cell value as a parameter use `[:param | ... ]` (explicit parameter block).
See the <<Iskara_Language_Reference.adoc#_blocks,Blocks>> section in the language reference.
